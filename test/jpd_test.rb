...
class JpdTest < Test::Unit::TestCase
  ...
  def test_update
    assert_equal({:a=>0.5, :b=>0.5}, @p.output_probs(nil))
    assert_equal({:a=>0.5, :b=>0.5}, @p.output_probs({[1,2] => [1,2]}))
    assert_equal({:a=>0.5, :b=>0.5}, @p.output_probs({[1,2] => [2]}))
    assert_equal({:a=>0.5, :b=>0.5}, @p.output_probs({[1,2] => [1]}))

    @p.update({[1,2] => [1]}, :a)

    assert_equal({:a=>0.75, :b=>0.25}, @p.output_probs(nil))
    assert_equal({:a=>0.75, :b=>0.25}, @p.output_probs({[1,2] => [1,2]}))
    assert_equal({:a=>0.5, :b=>0.5},   @p.output_probs({[1,2] => [2]}))
    ...
    assert((counts.keys - [:a, :b]).empty?)
    assert_in_delta(0.75, counts[:a] / (counts[:a] + counts[:b]), 0.05)
  end
end
